...
const StockIn = () => {
  ...
  const addItem = () => {
    if (selectedItem && quantity && costPerUnit) {
      const totalCost = (parseFloat(quantity) * parseFloat(costPerUnit)).toFixed(2);
      setData([
        ...data,
        [
          data.length + 1,
          ...
          quantity,
          totalCost,
          <button className="bg-red-500 text-white p-2 rounded shadow remove-btn" onClick={() => removeItem(data.length)}>Remove</button>
        ],
      ]);
      ...
      <div className="flex-grow">
        ...
        <div className="flex items-center space-x-4 mb-6">
          <div className="flex items-center space-x-2">
            <label htmlFor="supplier" className="font-bold">Supplier Name:</label>
            <input
              type="text"
              id="supplier"
          ...
          <div className="flex items-center space-x-2">
            <label htmlFor="receipt" className="font-bold">Receipt No:</label>
            <input
              type="text"
              id="receipt"
              placeholder="Enter text"
